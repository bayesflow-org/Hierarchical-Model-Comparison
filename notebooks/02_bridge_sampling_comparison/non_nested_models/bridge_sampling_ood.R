...
noise_data <- np$load("data/02_bridge_sampling_comparison/non_nested_models/noise_data_ood.npy", allow_pickle = TRUE)
n_data_sets = dim(noise_data)[1]
...
exp_file_comp <- sprintf("data/02_bridge_sampling_comparison/non_nested_models/%s_BF_BS_ood", exp_time)

write.table(comparison_results, file=exp_file_comp)

# Optional: Export number of divergent transitions per fit
#n_div_t_file = sprintf("data/02_bridge_sampling_comparison/non_nested_models/%s_BF_BS_n_div_trans_ood", exp_time)
