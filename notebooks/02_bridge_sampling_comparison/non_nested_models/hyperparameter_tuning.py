...
sys.path.append(os.path.abspath(os.path.join('../../..'))) # access sibling directories; different than .ipynb
...
n_combinations = len(bigger_network_128_units)*len(smaller_lr_5e_5)*n_runs_per_setting
    ...
    for smaller_lr in smaller_lr_5e_5:
        for run in range(n_runs_per_setting):
            ...
            if bigger_network_128_units == True:
                config = {"units": 128, "activation": "relu", "kernel_initializer": "glorot_uniform"}
                summary_net = bf.summary_networks.HierarchicalNetwork([
                    bf.networks.DeepSet(dense_s1_args=config, dense_s2_args=config, dense_s3_args=config), 
                    bf.networks.DeepSet(dense_s1_args=config, dense_s2_args=config, dense_s3_args=config)
                    ])
            else:
                summary_net = bf.summary_networks.HierarchicalNetwork([
