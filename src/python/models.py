...
class HierarchicalNormalSimulator:
    ...
    def gen_from_likelihood(self, theta, sigma2, n_obs):
        ...
        X = np.random.normal(
            loc=theta, scale=np.sqrt(sigma2), size=(n_obs, theta.shape[0])
        ).T
        return X

    def generate_single(
        self, model_index, n_clusters, n_obs, mu0=0, tau20=1, sigma_t=1, sigma_s=1
    ):
        """
        ...
        """
        theta, sigma2 = self.draw_from_prior(
            model_index, n_clusters, mu0, tau20, sigma_t, sigma_s
        )
        x_generated = self.gen_from_likelihood(theta, sigma2, n_obs)
        return x_generated[..., np.newaxis]

    def simulate(
        self, batch_size, n_clusters, n_obs, mu0=0, tau20=1, sigma_t=1, sigma_s=1
    ):
        """
        ...
        for b in range(batch_size):
            prior_sample = self.draw_from_prior(
                n_clusters, mu0, tau20, sigma_t, sigma_s
            )
    ...
    def generate_batch(
        self, batch_size, model_index, n_clusters, n_obs, n_vars=1, **kwargs
    ):
...
class HierarchicalSdtMptSimulator:
    ...
    def draw_from_prior(self, model_index, n_clusters):
        ...
        if model_index == 1:  # MPT model
            ...
            lambdas = RNG.uniform(
                0, 2, size=2
            )  # upper limit determines scaling of correlation matrix (low = low sds of random effects)
            Q = stats.invwishart.rvs(
                df=3, scale=np.identity(2)
            )  # df determines correlations between d and g (low = higher correlations, 3 = uniform)
    ...
    def generate_from_likelihood(self, p_h_m, p_f_m, n_clusters, n_obs):
        ...
        stim_cluster = np.repeat(
            [[1, 0]], repeats=n_trials_per_cat, axis=1
        )  # For 1 participant
        stim_data_set = np.repeat(
            stim_cluster, repeats=n_clusters, axis=0
        )  # For 1 data set

        # Create individual responses (0="new" / 1="old")
        X_h = RNG.binomial(
            n=1, p=p_h_m, size=(n_trials_per_cat, n_clusters)
        ).T  # Old items
        X_f = RNG.binomial(
            n=1, p=p_f_m, size=(n_trials_per_cat, n_clusters)
        ).T  # New items
    ...
    def generate_batch(
        self, batch_size, model_index, n_clusters, n_obs, n_vars=2, **kwargs
    ):
        """Generates a batch of simulated data sets from a fixed model."""
...
class MainSimulator:
    ...
    def draw_from_model_prior(self, batch_size, n_models, model_prior):
        ...
        model_indices = np.random.choice(
            model_base_indices, size=batch_size, p=model_prior
        )
    ...
    def simulate(self, batch_size, n_clusters, n_obs, n_vars, n_models, model_prior):
        ...
        for b in range(batch_size):
            X_gen[b] = self.simulator.generate_single(
                model_indices[b], n_clusters, n_obs
            )
